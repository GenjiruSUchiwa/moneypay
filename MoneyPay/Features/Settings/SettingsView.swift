...
struct SettingsView: View {
        ...
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    group("Maquette") {
                        link("Galerie des écrans", "square.grid.2x2") { GalleryView() }
                    }
            ...
            .safeAreaInset(edge: .top, spacing: 0) {
                HStack {
                    Text("Profil").font(.title1).tight(-0.6).foregroundStyle(Brand.ink)
                    Spacer()
                }
                .gutter()
                .padding(.top, 4)
        ...
        NavigationLink { ReferralView() } label: {
            HStack(spacing: 13) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text("Parrainez, gagnez 2 500 FCFA").font(.body).foregroundStyle(Brand.ink)
    ...
    private func toggleRow(_ title: String, _ icon: String, _ value: Binding<Bool>) -> some View {
        HStack(spacing: 13) {
            ...
            Text(title).font(.body).foregroundStyle(Brand.ink)
            Spacer()
