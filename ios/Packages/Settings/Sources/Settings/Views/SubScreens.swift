...
public struct ProfileView: View {
            ...
            VStack(alignment: .leading, spacing: 0) {
                HStack(spacing: 14) {
                    ...
                    VStack(alignment: .leading, spacing: 5) {
                        Text(store.user.fullName).font(.heading3).foregroundStyle(Brand.ink)
    ...
    private func kv(_ l: String, _ v: String) -> some View {
        HStack {
            ...
            Spacer()
            Text(v).font(.bodyReg).foregroundStyle(Brand.ink)
        }
                ...
                VStack(spacing: 0) {
                    ForEach(limits.indices, id: \.self) { i in
                        VStack(alignment: .leading, spacing: 9) {
                            HStack {
                                Text(limits[i].0).font(.bodyReg).foregroundStyle(Brand.ink)
                                Spacer()
    ...
    public var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 0) {
                ...
                .gutter()
                Text("Avec la confirmation systématique, chaque autorisation attend votre " +
                     "validation dans l'application. Sans réponse, elle est refusée au bout " +
                     "de 20 secondes.")
                    .font(.micro).foregroundStyle(Brand.inkFaint)
                    .fixedSize(horizontal: false, vertical: true)
...
public struct HelpView: View {
    ...
    private let faq: [(String, String)] = [
        ("Pourquoi mon paiement a-t-il été refusé ?",
         "Le plus souvent, votre solde FCFA ne couvrait pas le montant converti au moment " +
         "de l'autorisation. Rechargez puis réessayez. Chaque refus est facturé 220 FCFA."),
        ("Combien de temps pour recharger en Mobile Money ?",
         "MTN MoMo et Orange Money créditent votre wallet en quelques secondes. " +
         "Un virement bancaire prend 1 à 2 jours ouvrés."),
        ("Ma carte marche-t-elle sur tous les sites ?",
         "Partout où Visa et Mastercard sont acceptés en ligne. Les marchands qui exigent " +
         "une empreinte de caution — location de voiture, hôtels — refusent les cartes " +
         "virtuelles."),
        ("Quel taux de change est appliqué ?",
         "Le taux interbancaire du jour, majoré de 3 % de marge MoneyPay. Le détail est " +
         "affiché avant chaque conversion et sur chaque reçu."),
        ("Que se passe-t-il si je gèle une carte ?",
         "Toutes les autorisations sont refusées immédiatement. Les abonnements en cours " +
         "échoueront tant que la carte reste gelée. Vous pouvez la dégeler à tout moment.")
    ]
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            Button {
                                ...
                                HStack(alignment: .top, spacing: 10) {
                                    Text(item.element.0).font(.bodyReg).foregroundStyle(Brand.ink)
