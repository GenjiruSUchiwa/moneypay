...
public sealed class CreateSignUpCompletionHttpTests(MoniPayApi api) : MoniPayApiTest(api)
{
    ...
    public async Task A_retry_returns_200_and_a_replacement_session()
    {
        ...
        // The replaced session's credentials are dead: one live session, and it is the new one.
        using HttpResponseMessage replay = await SignUpFlow.PostRefreshAsync(
            Client,
            RefreshTokenOf(first),
            Guid.CreateVersion7());
    ...
    public async Task An_invalid_name_is_422_at_its_pointer(string firstName, string pointer)
    {
        ...
        Assert.Equal(pointer, Assert.Single(PointersOf(problem)));
        Assert.Equal(usersBefore, await Api.CountUsersAsync());
    }
    ...
    public async Task An_invalid_email_is_422_at_its_pointer(string email)
    {
        ...
        Assert.Equal(
            ...
            Assert.Single(PointersOf(problem)));
        Assert.Equal(usersBefore, await Api.CountUsersAsync());
    }
    ...
    /// <summary>
    /// Reads the session document once. A response body is a single-use stream, so a test that
    /// asserts on it must not ask for it twice.
    /// </summary>
    ...
    private static IReadOnlyList<string> PointersOf(Microsoft.AspNetCore.Mvc.ProblemDetails problem)
    {
        JsonElement errors = (JsonElement)(problem.Extensions["errors"]
            ?? throw new Xunit.Sdk.XunitException("The problem carried no errors member."));

        return [.. errors.EnumerateArray().Select(error => error.GetProperty("pointer").GetString()!)];
    }
}
